...
SELECT
  ...
  (SELECT COUNT(*) FROM subscriptions WHERE status = 'canceled' AND canceled_at > CURRENT_TIMESTAMP - INTERVAL '30 days') AS recent_cancellations,
  ...
  (SELECT COUNT(DISTINCT user_id) FROM recent_session_states WHERE updated_at > CURRENT_TIMESTAMP - INTERVAL '7 days') AS active_users_last_7_days
FROM auth.users u;
...
FROM beta_codes b
LEFT JOIN trial_subscriptions t ON t.trial_code = b.code
...
SELECT
  ...
  (SELECT COUNT(DISTINCT user_id) FROM subscriptions WHERE status = 'active') AS total_premium_users,
  ROUND(
    COUNT(DISTINCT rs.user_id)::numeric / NULLIF((SELECT COUNT(*) FROM auth.users), 0)::numeric * 100,
    1
  ) AS daily_active_user_percentage
FROM recent_session_states rs
LEFT JOIN saved_settings ss ON ss.updated_at::date = date_trunc('day', rs.updated_at)::date
WHERE rs.updated_at > CURRENT_DATE - INTERVAL '30 days'
GROUP BY activity_date
ORDER BY activity_date DESC;

-- Grant access to the service role (using proper view permissions)
...
-- Revoke access from public/anon users
REVOKE ALL ON public.daily_metrics_summary FROM anon, public;
REVOKE ALL ON public.subscription_analytics FROM anon, public;
REVOKE ALL ON public.beta_code_performance FROM anon, public;
REVOKE ALL ON public.user_activity_metrics FROM anon, public;

-- Add comments to explain what each view does
