...
-- Set the proper permissions on views (using GRANT instead of RLS policies)
-- First, set ownership to postgres (superuser)
...
-- Grant SELECT permissions to service_role only
...
-- Revoke any broader permissions that might exist
REVOKE ALL ON public.daily_metrics_summary FROM PUBLIC;
REVOKE ALL ON public.subscription_analytics FROM PUBLIC;
REVOKE ALL ON public.beta_code_performance FROM PUBLIC;
REVOKE ALL ON public.user_activity_metrics FROM PUBLIC;

-- Create a function that will check if a user is admin for accessing analytics
-- This will be used by the application code
...
BEGIN
  ...
  RETURN current_email IN (
    'danabrams999@yahoo.com',
    'immortality.bytes.book@gmail.com',
    'dan@corpania.com'
  );
END;
