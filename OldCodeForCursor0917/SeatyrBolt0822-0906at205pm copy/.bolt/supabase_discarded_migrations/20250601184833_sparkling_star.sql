...
-- Now that views are created, set up the RLS policies
-- We need to do this in a separate step to avoid the error
...
DO $$
BEGIN
  -- Create policies for daily_metrics_summary
  IF NOT EXISTS (
    SELECT FROM pg_policies 
    WHERE tablename = 'daily_metrics_summary' AND policyname = 'Only service role can view daily metrics summary'
  ) THEN
    CREATE POLICY "Only service role can view daily metrics summary" 
      ON public.daily_metrics_summary 
      FOR SELECT TO service_role USING (true);
  END IF;

  -- Create policies for subscription_analytics
  IF NOT EXISTS (
    SELECT FROM pg_policies 
    WHERE tablename = 'subscription_analytics' AND policyname = 'Only service role can view subscription analytics'
  ) THEN
    CREATE POLICY "Only service role can view subscription analytics" 
      ON public.subscription_analytics 
      FOR SELECT TO service_role USING (true);
  END IF;

  -- Create policies for beta_code_performance
  IF NOT EXISTS (
    SELECT FROM pg_policies 
    WHERE tablename = 'beta_code_performance' AND policyname = 'Only service role can view beta code performance'
  ) THEN
    CREATE POLICY "Only service role can view beta code performance" 
      ON public.beta_code_performance 
      FOR SELECT TO service_role USING (true);
  END IF;

  -- Create policies for user_activity_metrics
  IF NOT EXISTS (
    SELECT FROM pg_policies 
    WHERE tablename = 'user_activity_metrics' AND policyname = 'Only service role can view user activity metrics'
  ) THEN
    CREATE POLICY "Only service role can view user activity metrics" 
