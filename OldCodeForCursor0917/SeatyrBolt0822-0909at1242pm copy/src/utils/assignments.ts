...
/**
 * Normalizes assignment input and returns both the CSV and unknown tokens.
 * This is used by the seating algorithm to surface warnings for unknown table references.
 * 
 * @param rawInput The raw string input from the user
 * @param tables The application's list of tables to resolve names against
 * @returns Object with normalized CSV and list of unknown tokens
 */
export function normalizeAssignmentInputToIdsWithWarnings(
  rawInput: string,
  tables: Pick<Table, 'id' | 'name'>[]
): { idsCsv: string; unknownTokens: string[] } {
  if (!rawInput || typeof rawInput !== 'string') return { idsCsv: '', unknownTokens: [] };
  ...
  // Return sorted, deduped CSV and unknown tokens
  return {
    idsCsv: Array.from(ids).sort((a, b) => a - b).join(','),
    unknownTokens
  };
}
