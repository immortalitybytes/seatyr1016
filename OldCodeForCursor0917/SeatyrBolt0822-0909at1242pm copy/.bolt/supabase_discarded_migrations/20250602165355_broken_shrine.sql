...
-- Set ownership to postgres to allow access control
...
-- Revoke access from public role for security
...
-- NOTE: We don't create RLS policies on views as they are not tables.
-- Instead, we control access via GRANTs (above).
-- Views inherently respect the RLS policies of their underlying tables.