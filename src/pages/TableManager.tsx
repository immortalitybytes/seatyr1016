...
const TableManager: React.FC = () => {
  ...
  const updateConstraints = useCallback((guestId: GuestID, names: string[], type: 'must' | 'cannot') => {
    ...
    const nextConstraints = { ...state.constraints[type] };
  ...
  return (
    <div className="space-y-6">
      <Card title="Table Management" icon={<TableIcon className="w-5 h-5" />}>
      ...
      <Card title="Guest Assignments & Constraints" icon={<MapPin className="w-5 h-5" />}>
            ...
            {guestListOpen && (
              <div className="space-y-6">
                ...
                {sortedGuests.map(guest => {
                  ...
                  const mustNames = (state.constraints.must?.[guest.id] || []).map(id => state.guests.find(g => g.id === id)?.name || id);
                  const cannotNames = (state.constraints.cannot?.[guest.id] || []).map(id => state.guests.find(g => g.id === id)?.name || id);
