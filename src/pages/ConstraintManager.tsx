...
import { ClipboardList, Info, AlertCircle, ChevronLeft, ChevronRight, ArrowDownAZ, ChevronDown, ChevronUp, X, Download, AlertTriangle } from 'lucide-react';
...
const ConstraintManager: React.FC = () => {
  ...
  const sortedGuests = useMemo(() => {
    ...
    switch (sortOption) {
      ...
      case 'current-table': {
        ...
        const currentPlan = plans[state.currentPlanIndex];
  ...
  const handleCellClick = useDebouncedCallback((g1: string, g2: string) => {
    ...
    if (isCannot) {
      ...
      dispatch({ type: 'SET_ADJACENT', payload: { a: id1, b: id2 } });
    } else if (isAdjacent) {
      dispatch({ type: 'SET_ADJACENT', payload: { a: id1, b: id2 } });
    } else {
      dispatch({ type: 'SET_CONSTRAINT', payload: { a: id1, b: id2, type: 'cannot', removeType: 'must' } });
    }
  }, 300);
  ...
  return (
    <div className="space-y-6">
      <Card title="Your Rules (Constraints)" icon={<ClipboardList className="w-5 h-5" />}>
        <div className="mb-4 space-y-3">
          ...
          {/* SSoT-COMPLIANT Conflict/Error Display (Reads from state.planErrors) */}
          {showConflicts && state.planErrors.length > 0 && (
            <div className="mt-4 mb-4 bg-red-50 border border-red-200 rounded-md p-3">
              ...
              <ul className="space-y-2">
                {state.planErrors.map((error: ValidationError, index) => (
