-- ============================================================================
-- Phase A: Database Hardening for Seatyr (v3 - ROBUST)
-- ============================================================================
...
-- Version: 3.0 (adapted to actual Supabase schema)
-- ============================================================================

BEGIN;

-- ============================================================================
-- PREFLIGHT: Preview Duplicate Counts
-- ============================================================================

SELECT 'PREFLIGHT DUPLICATE COUNTS' AS section;

SELECT 'subscriptions dupes' AS scope, COUNT(*) AS dupe_user_count FROM (
  SELECT user_id FROM public.subscriptions WHERE user_id IS NOT NULL GROUP BY user_id HAVING COUNT(*) > 1
) s
UNION ALL
SELECT 'trial_subscriptions dupes', COUNT(*) FROM (
  SELECT user_id FROM public.trial_subscriptions WHERE user_id IS NOT NULL GROUP BY user_id HAVING COUNT(*) > 1
) t
UNION ALL
SELECT 'recent_session_states dupes', COUNT(*) FROM (
  SELECT user_id FROM public.recent_session_states WHERE user_id IS NOT NULL GROUP BY user_id HAVING COUNT(*) > 1
) rs
UNION ALL
SELECT 'recent_session_settings dupes', COUNT(*) FROM (
  SELECT user_id FROM public.recent_session_settings WHERE user_id IS NOT NULL GROUP BY user_id HAVING COUNT(*) > 1
) rset;
...
-- Assumes: id column exists as primary key
WITH ranked_subs AS (
  SELECT id,
         ROW_NUMBER() OVER (
...
-- Assumes: id column exists, expires_on is the only expiry field
...
-- Recent session states: keep newest per user
-- Uses ctid since table likely has no id column; falls back to updated_at only
...
-- Recent session settings: keep newest per user
-- This table HAS an id column, so we can use it for deterministic tie-breaking
...
CREATE UNIQUE INDEX IF NOT EXISTS trial_subscriptions_user_unique 
  ON public.trial_subscriptions(user_id) WHERE user_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS recent_session_states_user_unique 
  ON public.recent_session_states(user_id) WHERE user_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS recent_session_settings_user_unique 
  ON public.recent_session_settings(user_id) WHERE user_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_saved_settings_user_id 
  ON public.saved_settings (user_id) WHERE user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_saved_settings_user_updated_at 
  ON public.saved_settings (user_id, updated_at DESC) WHERE user_id IS NOT NULL;

-- Add indexes to support deduplication queries and common filters
CREATE INDEX IF NOT EXISTS idx_subscriptions_user_period 
  ON public.subscriptions(user_id, current_period_end DESC NULLS LAST) WHERE user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_trial_subscriptions_user_expires 
  ON public.trial_subscriptions(user_id, expires_on DESC NULLS LAST) WHERE user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_recent_states_user_updated 
  ON public.recent_session_states(user_id, updated_at DESC NULLS LAST) WHERE user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_recent_settings_user_updated 
  ON public.recent_session_settings(user_id, updated_at DESC NULLS LAST, id DESC) WHERE user_id IS NOT NULL;
...
-- Verify exact policies were created
DO $$
DECLARE
